...
<Tabs groupId="platform" items={['macOS', 'Windows', 'Linux', 'Docker']}>
  ...
  <Tab value="Linux">
    Install with Flatpak from [FlatPark](https://flatpark.org/apps/org.mutantcat.nexdb/) (works on any distribution):
    ...
    ```bash
    flatpak update org.mutantcat.nexdb
    ```
...
<Callout type="info">
  First-time contributors should follow the contribution guide in the [NexDB repository](https://github.com/Mutantcat-Working-Group/NexDB) for complete platform setup, forking, issue claiming, testing, and pull request steps.
</Callout>
