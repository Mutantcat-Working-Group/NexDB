...
- 90+ database types across relational, NoSQL, time-series, and message queues.
- Native drivers first, with Agent/JDBC extensions covering the long tail of databases.
- SSH tunnel, auto-reconnect, and encrypted config import/export.

#### Querying and data operations

- Query editor, virtualized data grid, and explain plan.
- Schema tools, ER diagram, and schema diff.
- Import CSV or Excel, migrate between databases, export a database, compare table data, run `.sql` files, and drag and drop Parquet, CSV, or JSON files for instant preview.
- Redis and MongoDB browsers.

#### AI SQL assistant

- Natural-language SQL generation, explanation, optimization, and error fixing, with built-in safety checks.
- Works with Claude, OpenAI, local models via Ollama, and any OpenAI-compatible endpoint.

#### MCP server

- Claude Code, Cursor, Windsurf, and other AI coding agents can query your configured databases.
- Three execution permissions: read-only / data read-write / full access.
...
- Description: lets MCP-compatible agents list connections, browse tables, execute SQL, and open tables directly in NexDB. Service name: `org.mutantcat.nexdb`.
