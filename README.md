...
- 90+ 数据库类型：关系型、NoSQL、时序、消息队列全覆盖。
- 原生驱动为主，Agent/JDBC 扩展覆盖长尾数据库。
- SSH 隧道、断线重连、连接配置加密导入导出。

#### 查询与数据操作

- 查询编辑器、虚拟滚动数据表格、执行计划。
- Schema 工具、ER 图、Schema 对比。
- 导入 CSV/Excel、跨库迁移、完整导出、数据对比、执行 `.sql` 文件，拖入 Parquet、CSV、JSON 文件即可即时预览。
- Redis / MongoDB 专项浏览器。

#### AI SQL 助手

- 自然语言生成 SQL、解释、优化与纠错，内置安全检查。
- 支持 Claude、OpenAI、Ollama 本地模型及 OpenAI 兼容端点。

#### MCP Server

- Claude Code、Cursor、Windsurf 等 AI 编程助手可直接查询已配置的数据库。
- 三档执行权限：只读 / 数据读写 / 完全访问。
