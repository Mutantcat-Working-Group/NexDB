...
test("common agent test changes do not bump driver versions", () => {
  ...
  const result = evaluateAgentVersionBump({
    ...
    changedFiles: ["agents/common/src/test/java/org/mutantcat/agent/AbstractJdbcAgentTest.java"],
    ...fixture,
  });
...
test("manual agent versions are preserved while other changed modules auto bump", () => {
  ...
  const result = evaluateAgentVersionBump({
    ...
    changedFiles: ["agents/common/src/main/java/org/mutantcat/agent/JdbcExecutor.java", "agents/versions.json"],
    ...fixture,
  });
